---
title: "Bio Bayesian Trials Together"
author: "Abby"
date: "2024-11-29"
...
bayes_rm <- generalTestBF(Symbiont.Counts..symbiont.cm2. ~ Turb * 
                            Temp + Genotype, 
...
```{r}
#So now we make our Bayesian Models
bayes_rm.change.symbiont <- generalTestBF(change.symbiont ~ Turb * 
                            Temp + Genotype, 
                          data = cambios, whichRandom = "Genotype", 
                          neverExclude = "Genotype")
```
...
Prints out our model and tells us what's good.
```{r}
bayes_rm.change.symbiont
```

```{r}
plot(bayes_rm.change.symbiont)
```
Everything less likely than just genotype
