...
bayes_rm <- generalTestBF(Chl.a.C.2 ~ Turb * 
                            Temp * Phase + Genotype, 
...
bayes_rm
```
1.064004 times more likely than just Genotype
...
p2 <- ggplot(cambios, aes(x = Turb, y = Chl.a.C.2, label = Genotype))+
  ...
  labs(fill = "Treatment")+
  xlab("Turbidity (NTU)")+
  ylab("Total Chlorophyll μg/ml/cm^2")+
  ggtitle("Total Chlorophyll by Turbidity")

p2
```
Looks quite similar to the chlorophyll a graph.

```{r}
ggsave(p2, 
       filename = "TotalChlCombined.png",
       device = "png")
```
...
```{r}
#So now we make our Bayesian Models
bayes_rm.change.Chl.a.C.2<- generalTestBF(change.Chl.a.C.2 ~ Turb * 
                            Temp*Phase + Genotype, 
                          data = cambios, whichRandom = "Genotype", 
                          neverExclude = "Genotype")
```
...
Prints out our model and tells us what's good.
```{r}
bayes_rm.change.Chl.a.C.2
```

```{r}
plot(bayes_rm.change.Chl.a.C.2)
```
Slightly less likely than genotype
...
#And then we make our graph
p3 <- ggplot(cambios, aes(x = Turb, y = change.Chl.a.C.2, label = Genotype))+
  geom_flat_violin(data= cambios, aes(x= Turb, 
                                          y = change.Chl.a.C.2, 
                                        fill = Treatment), 
                   position = position_nudge(x = 0, y = 0), adjust = 1.5, trim = TRUE, alpha = .5, colour = NA)+
  geom_text_repel(aes(x= Turb, y = change.Chl.a.C.2, 
                                    colour = Treatment), 
                  nudge_x = -0.3, show.legend = FALSE)+
  ...
  labs(fill = "Treatment")+
  xlab("Turbidity (NTU)")+
  ylab("Change in Chlorophyll a to Chlorophyll C2")+
  ggtitle("Change Chlorophyll a to Chlorophyll C2 by Turbidity")

p3
```
This seems to make genotype 84 and 101 in zero turb treatments less of outliers.
