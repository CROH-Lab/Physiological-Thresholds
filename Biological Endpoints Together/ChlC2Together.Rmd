...
bayes_rm <- generalTestBF(Chl.C.2 ~ Turb * 
                            Temp * Phase + Genotype, 
...
Everything is less likely than genotype

What about when we look at offset
```{r}
#So now we make our Bayesian Models
bayes_rm.change.change.Chl.C.2 <- generalTestBF(change.Chl.C.2 ~ Turb * 
                            Temp*Phase + Genotype, 
                          data = cambios, whichRandom = "Genotype", 
                          neverExclude = "Genotype")
```
But we'll still graph it so it looks prettier
...
p2 <- ggplot(cambios, aes(x = Turb, y = Chl.C.2, label = Genotype))+
  ...
  labs(fill = "Treatment")+
  xlab("Turbidity (NTU)")+
  ylab("Chlorophyll C2 μg/ml/cm^2")+
  ggtitle("Chlorophyll C2 by Turbidity")
...
```{r}
ggsave(p2, 
       filename = "ChlCCombined.png",
       device = "png")
```

Prints out our model and tells us what's good.
```{r}
bayes_rm.change.change.Chl.C.2
```

```{r}
plot(bayes_rm.change.change.Chl.C.2)
```
Still nothing more likely than just genotype.
