...
Cham <- 7 #Chamber labeled to prevent subsequent confusion
...
```
Something like day/night cycles
...
```
Almost all respiration all the time
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 4 Chamber 7") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
Occasional moments of net photosynthesis
...
#write.csv(corrected.data, "Trial4Chamber7NoBG.csv")
