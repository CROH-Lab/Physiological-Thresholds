...
library(respR)
library(tidyr)
raw.data <- read.csv("Trial4Resp.csv") #load raw trial data
...
Trial <- 4 #Experiment labeled to prevent subsequent confustion
Cham <- 5 #Chamber labeled to prevent subsequent confusion
...
```
Seems to be a slightly happier coral
...
```
This time we have 85 measurement phases
...
m.end <- as.numeric(rownames(as.data.frame(coral.resp)[which(as.data.frame(coral.resp)$Phase=="M85  ", arr.ind=TRUE)[1],]))
...
```
Photosynthesis looks less impressive on closer examination
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 4 Chamber 5") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
More time in the chamber gets you a less happy coral
...
old.results <- read.csv("Trial4ResultswithBackground.csv")

#So the cycle column isn't labeled the way we want it, but that's fixable
names(old.results)[names(old.results) == 'Cycle'] <- 'AutoResp.Cycle.Loop'
...
#But we have some weird extra columns that may mess up merging later
colnames(corrected.data)
```

How to get rid of the columns we don't like
```{r}
corrected.data <- select(corrected.data, -X, -X.1, -X.2, -X.3, -X.4, 
                                -Experiment.starts.at.cycle.11)
```

```{r}
#Write a csv of our data. We'll put them all together later
#write.csv(corrected.data, "Trial4Chamber5NoBG.csv")
