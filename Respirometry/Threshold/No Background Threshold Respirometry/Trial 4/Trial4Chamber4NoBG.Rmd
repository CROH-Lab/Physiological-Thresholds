...
Cham <- 4 #Chamber labeled to prevent subsequent confusion
...
```
Really falls off at the end. I wonder if Briana took those loops out
...
```
Respiring more than photosynthesizing
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 4 Chamber 4") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
So the giant drop might be something real
...
#write.csv(corrected.data, "Trial4Chamber4NoBG.csv")
