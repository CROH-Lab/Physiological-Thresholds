---
title: "Daytime Multistressor Respirometry 20-Dec-2024"
author: "Abby Cannon"
date: "2024-10-29"
...
datos has 2142 rows so we're missing some loops. This may be a bigger or smaller problem depending on where the missing values are.
...
oxy.by.day <- dia %>%
  ...
  baja.curva.3()
```
Now we need to make some labels to keep Dr. Bahr happy with our graph
```{r}
#First we make a new data frame with our new labels
nuevas.etiquetas <- data.frame(Treatment = unique(oxy.by.day$Treatment), 
                               etiqueta = c("0 NTU, 27ºC", "29 NTU, 27ºC", "0 NTU, 30ºC", "29 NTU, 30ºC"))

#Then we attach it to our old one
oxy.by.day = merge(oxy.by.day, nuevas.etiquetas, by = "Treatment")
```
...
sum.oxy.by.day <- summarySE(oxy.by.day, measurevar = "oxy",
                       groupvars=c("Treatment", "Day.of.Exp", "etiqueta"), na.rm = TRUE)
...
p2 <- ggplot(oxy.by.day, aes(x=Day.of.Exp, y = oxy))+
  ...
  geom_point(aes(x=factor(Day.of.Exp, levels = 1:3), y = oxy, 
                                    colour = etiqueta, shape = etiqueta), 
             position = position_jitter(width = .2), size = 2)+
  geom_line(data = sum.oxy.by.day, aes(x = as.numeric(Day.of.Exp)+.25, y = oxy_mean, 
                                       group = etiqueta, colour = etiqueta), 
            linetype = 3)+
  scale_color_manual(values = c("black", "#FF0000", "#000080",  "#00CD00"))+
  scale_fill_manual(values = c("black", "#FF0000", "#000080",  "#00CD00"))+
...
p2
```
Now let's get that plot ready
```{r}
ggsave(p2, 
       filename = "ThresholdDaytimeRespirometry20Dec2024.png",
       device = "png")
```
...
plot(bayes_rm)
```
So the most likely is Day.of.Exp + Temperature + Day.of.Exp:Temperature + Turbidity + Day.of.Exp:Turbidity + ID
So no interaction between temperature and turbidity although the both interact with day of experiment
...
look.at.this
```
So our model is 2.94864e+11 times as likely as a model just based on ID.
