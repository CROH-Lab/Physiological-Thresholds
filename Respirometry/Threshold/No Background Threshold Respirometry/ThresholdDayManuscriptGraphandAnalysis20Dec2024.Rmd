---
title: "Threshold by Day and Treatment, No Genotype, Better Graph Labels"
author: "Abby Cannon"
date: "2024-12-20"
...
sum.oxy.by.day <- summarySE(oxy.by.day, measurevar = "oxy",
                       groupvars=c("Treatment", "Day.of.Exp", "etiqueta"))
...
```{r}
ggsave(p2, 
       filename = "DaylightThresholdIntegrated20-Dec-2024.png",
       device = "png")
```
...
And now we take out genotype, because it's a source of too many problems
```{r}
#So now we make our Bayesian Models
bayes_rm <- generalTestBF(oxy ~ Day.of.Exp * Treatment + ID,
...
plot(bayes_rm)
```
Day.of.Exp + Treatment + ID is the most likely
...
bfmat[,5] #Prints out the 19th column, which should show everything divided by ID
```
Day.of.Exp + Treatment + ID is 24104.4 times more likely than just ID.
