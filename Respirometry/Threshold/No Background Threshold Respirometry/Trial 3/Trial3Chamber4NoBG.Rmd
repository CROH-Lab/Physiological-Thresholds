...
library(respR)
library(tidyr)
raw.data <- read.csv("Trial3Resp.csv") #load raw trial data
...
Trial <- 3 #Experiment labeled to prevent subsequent confustion
...
```
Looks messy, but sometimes it be that way
...
```
This time we have 87 measurement phases
...
m.end <- as.numeric(rownames(as.data.frame(coral.resp)[which(as.data.frame(coral.resp)$Phase=="M87  ", arr.ind=TRUE)[1],]))
...
```
Finally seeing some actual oxygen production
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 3 Chamber 4") + geom_hline(aes(yintercept=0)) +
...
old.results <- read.csv("Trial3ResultswithBackground.csv")
...
#write.csv(corrected.data, "Trial3Chamber4NoBG.csv")
