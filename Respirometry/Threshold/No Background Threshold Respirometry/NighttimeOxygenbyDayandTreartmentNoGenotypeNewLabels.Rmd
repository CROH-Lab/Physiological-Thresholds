---
title: "Code used for nighttime respirometry figure and analysis in manuscript"
author: "Abby Cannon"
date: "2024-12-19"
output: html_document
---

title: "Nighttime Threshold Integrated Offset from Night Max"
author: "Abby Cannon"
date: "2024-10-08"
output: html_document
---
...
```{r}
unique(datos$Time.of.Day) #Only "day" and "night" so no trimming needed
```
...
Now we aggregate by day and for night
```{r}
...
baja.curva.3 <- function(noche) {
  noche %>%
    summarise(oxy = sum(diff(Hour[order(Hour)])*
                          rollmean(Interpolated.MO2[order(Hour)],2)))
     }
...
nuevas.etiquetas <- data.frame(Treatment = unique(oxy.by.night$Treatment), 
                               etiqueta = c("0 NTU, 27ºC", "15 NTU, 27ºC", 
                                            "29 NTU, 27ºC", "50 NTU, 27ºC"))

#Then we attach it to our old one
oxy.by.night = merge(oxy.by.night, nuevas.etiquetas, by = "Treatment")
```
...
p2
```
Still looks like there's not much to be said for anything.

saves our graph, which is useful for the future
```{r}
ggsave(p2, 
       filename = "NightimeRespirometryPhase1SameAxisasPhase2NewLabels.png",
       device = "png")
```
...
So ID only is the most likely model, which tracks.
