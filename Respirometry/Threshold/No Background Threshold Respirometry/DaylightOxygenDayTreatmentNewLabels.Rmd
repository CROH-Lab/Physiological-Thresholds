---
title: "Used for daytime oxygen figure in manuscript"
author: "Abby"
date: "2024-12-19"
...
library(tidyr) #May need this again
...
p2
```
So it seems like even 15 NTU causes some problems when we don't adjust for backgrounds

Now let's get that plot ready
```{r}
ggsave(p2, 
       filename = "DaylightThresholdIntegratedNewLabels.png",
       device = "png")
```

```{r}
ggsave(p2, 
       filename = "DaylightThresholdIntegrated.pdf",
       device = "pdf")
```
...
plot(bayes_rm)
```
So our most likely model is Day.of.Exp + Treatment + Genotype + Treatment:Genotype + ID
...
```
So I think we want number 14 this time

```{r}
#Gets us a nice data frame to look at for future reference.
c <- as.matrix(bfmat[14,])

x <- as.data.frame(c) #63296.62 times more likely than just ID and otype + ID otype + ID 6.541561 more likely than just Day and ID
