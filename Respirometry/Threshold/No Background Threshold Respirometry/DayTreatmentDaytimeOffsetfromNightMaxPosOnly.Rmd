...
```{r}
dos <- read.csv("Trial2FromNightMaxPosOnly.csv")
tres <- read.csv("Trial3FromNightMaxPosOnly.csv")
cuatro <- read.csv("Trial4FromNightMaxPosOnly.csv")
nueve <- read.csv("Trial9FromNightMaxPosOnly.csv")
```
...
Now we need some info for calcification
```{r}
genes.and.trials <- data.frame(Genotype = datos$Genotype, Trial = datos$Trial)
u.gene.t <- unique(genes.and.trials)
#write.csv(u.gene.t, "TrialandGenotypes.csv")
```
...
Looks like 50 is always a problem and all turbidity treatments diverge increasingly from zero over time.

Now let's get that plot ready
```{r}
ggsave(p2, 
       filename = "DaylightThresholdIntegratedOffsetfromNightMaxPosOnly.png",
       device = "png")
```
...
class(oxy.by.day$Treatment) #Integer, probably okay
```

And day of experiment
```{r}
class(oxy.by.day$Day.of.Exp) #Also integer
```
...
bayes_rm <- generalTestBF(oxy ~ Day.of.Exp * Treatment * Genotype + ID,
                          data = oxy.by.day, whichRandom = "ID", neverExclude="^ID$")
```
...
plot(bayes_rm)
```
So most likely model is Day.of.Exp + Treatment + Genotype + Treatment:Genotype + ID

Now let's get our details about how much better it is than our other model
```{r}
#Gets every model diveded by evey other model
bfmat <- bayes_rm/bayes_rm

bfmat[,19] #Prints out the 19th column, which should show everything divided by ID
```
183603.2 times more likely than just ID. Genotype apparently still matters. Let's see how it looks when graphed

```{r}
#Gets us a nice data frame to look at for future reference.
c <- as.matrix(bfmat[14,])

x <- as.data.frame(c) 183603.2

```

So now let's see what happens when we plot genotypes 
```{r}
library(readr)
library(ggplot2)
library(dplyr)
...
p3 <- ggplot(oxy.by.day, aes(x=Day.of.Exp, y = oxy))+
  ...
  #scale_fill_manual(values = c("black", "#1E90FF", "#000080", "#881B1B"))+
  scale_shape_manual(values = c(15, 3, 17, 4))+
  labs(color = "Genotype", fill= "Genotype", shape = "Target NTU")+ 
  geom_hline(yintercept = 0, color = "black", linetype = "dashed")+
  #theme_cowplot()+
  theme(panel.background = element_rect(fill = "white", colour = "black"))+
  xlab("Day of Experiment")+
...
```{r}
ggsave(p3, 
       filename = "GenotypeEasterRespirometryDaymare.png",
       device = "png")
```