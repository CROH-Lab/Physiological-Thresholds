...
```
Looks like we got some daytime photosynthesis
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 9 Chamber 2") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
Does okay sometimes, but seems to be doing a lot more respiration
...
#write.csv(corrected.data, "Trial9Chamber2NoBG.csv")
