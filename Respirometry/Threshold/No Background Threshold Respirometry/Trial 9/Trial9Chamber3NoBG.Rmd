...
```
Looks like we have a few weird ones mixed in
...
```
Looks like it's still net O2 uptake
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 9 Chamber 3") + geom_hline(aes(yintercept=0)) +
...
#write.csv(corrected.data, "Trial9Chamber3NoBG.csv")
