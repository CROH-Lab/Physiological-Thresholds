---
title: "Trial 9 Night Max Is Zero"
author: "Abby Cannon"
date: "2024-10-08"
...
unique(datos$Time.of.Day) #Looks like we have some spacing issues. but that's
#fixable
library(stringr) #The package we neer 
datos$Time.of.Day <- str_trim(datos$Time.of.Day, side = c("both"))
unique(datos$Time.of.Day) #Alright. Now we can subset, no problem
...
ggplot(calc, aes(x= Hour, y= MO2.from.Dark.Max, 
  ...
  theme_bw()
```
So it doesn't look like we got the predicted turbidity effect this time.

Let's get our data ready for subsequent programming
```{r}
write.csv(calc, "Trial9FromNightMaxPosOnly.csv")
```
