...
library(respR)
library(tidyr)
raw.data <- read.csv("Trial9Resp.csv") #load raw trial data
...
Trial <- 9 #Experiment labeled to prevent subsequent confustion
...
```
Somewhat cyclic, but not as predictable as it should be
...
```
This time we have 86 measurement phases
...
```
More respiration as time goes on
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 9 Chamber 4") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
Kind of lost the plot, but así es la vida
...
old.results <- read.csv("Trial9ResultswithBackground.csv")
...
#write.csv(corrected.data, "Trial9Chamber4NoBG.csv")
