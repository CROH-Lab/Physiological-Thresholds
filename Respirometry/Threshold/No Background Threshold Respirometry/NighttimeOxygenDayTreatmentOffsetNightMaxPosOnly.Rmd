...
```{r}
ggsave(p2, 
       filename = "NighttimeThresholdIntegratedOffsetfromNightMaxPosOnly.png",
       device = "png")
```
...
plot(bayes_rm)
```
So Genotype + IS is apparently the most likely

#Gets every model diveded by evey other model
```{r}
bfmat <- bayes_rm/bayes_rm

bfmat[5,] #Prints out the first column, which lets us looks for our most likely model. It will have the biggest number
```
Genotype is 8.644923 times more likely than just ID
...
sum.oxy.by.night.genotype <- summarySE(oxy.by.night, measurevar = "oxy",
                       groupvars=c("Genotype", "Day.of.Exp"))

p3 <- ggplot(oxy.by.night, aes(x=Day.of.Exp, y = oxy))+
  ...
  geom_point(aes(x=factor(Day.of.Exp, levels = 1:3), y = oxy, 
             ...
             position = position_jitter(width = .2), size = 2)+
  geom_line(data = sum.oxy.by.night.genotype, aes(x = as.numeric(Day.of.Exp)+.25, y = oxy_mean, 
  ...
  xlab("Day of Experiment")+
  ylab("Oxygen Produced mg/L/12 hrs")+
  ggtitle("Nighttime Oxygen Production by Day and Genotype")
...
```{r}
ggsave(p3, 
       filename = "GenotypeEasterRespirometryNightmare.png",
       device = "png")
```