...
sum.oxy.by.day <- summarySE(oxy.by.day, measurevar = "oxy",
                       groupvars=c("Treatment", "Day.of.Exp"), na.rm = TRUE)

#And then we make our graph
p2 <- ggplot(oxy.by.day, aes(x=Day.of.Exp, y = oxy))+
  
  geom_flat_violin(data=oxy.by.day, aes(x=factor(Day.of.Exp, levels = 1:3), y = oxy, 
  ...
  geom_line(data = sum.oxy.by.day, aes(x = as.numeric(Day.of.Exp)+.25, y = oxy_mean, 
  ...
  ggtitle("Daylight Oxygen Production by Day and Treatment")

p2
```

Alright looks pretty good, but we'll have to analyze.
But this way we get our image
```{r}
ggsave(p2, 
       filename = "DaytimeTurbTempIntegrated.png",
       device = "png")
```
...
oxy.by.day$ID <- paste(oxy.by.day$Trial, oxy.by.day$Chamber, sep = "-")
#And it needs to be a factor
oxy.by.day$ID <- as.factor(oxy.by.day$ID)
```

And we'll need to make columns for temperature and turbidity for analysis and I choose to do this in a kind of silly way
```{r}
oxy.by.day$Temperature <- oxy.by.day$Treatment
class(oxy.by.day$Temperature) #Already character
#So now we can make some substitutions to get it what it needs to be
unique(oxy.by.day$Temperature) #Figuring our what we're actually substituting
#"Control"            "High NTU"           "High Temp"          "High Temp High NTU"

#Now we make the substitutions
oxy.by.day$Temperature <- gsub("High Temp High NTU", "30", oxy.by.day$Temperature)
#And we have to do high temp high NTU first, because it has the names of others in it
oxy.by.day$Temperature <- gsub("Control", "27", oxy.by.day$Temperature)
oxy.by.day$Temperature <- gsub("High NTU", "27", oxy.by.day$Temperature)
oxy.by.day$Temperature <- gsub("High Temp", "30", oxy.by.day$Temperature)

#Now we make it numeric
temperatura <- as.numeric(oxy.by.day$Temperature)

#And then we put it back in our data frame
oxy.by.day$Temperature <- temperatura
```

And now we'll have to do something very similar for turbidity
```{r}
oxy.by.day$Turbidity <- oxy.by.day$Treatment

#Now we make the substitutions
oxy.by.day$Turbidity <- gsub("High Temp High NTU", "29", oxy.by.day$Turbidity)
#And we have to do high temp high NTU first, because it has the names of others in it
oxy.by.day$Turbidity <- gsub("Control", "0", oxy.by.day$Turbidity)
oxy.by.day$Turbidity <- gsub("High NTU", "29", oxy.by.day$Turbidity)
oxy.by.day$Turbidity <- gsub("High Temp", "0", oxy.by.day$Turbidity)

#Now we make it numeric
turbidez <- as.numeric(oxy.by.day$Turbidity)

#And then we put it back in our data frame
oxy.by.day$Turbidity <- turbidez
```

And day of experiment
```{r}
class(oxy.by.day$Day.of.Exp) #Numeric. That should be fine
```

```{r}
#And genotype
class(oxy.by.day$Genotype) #Integer. We want it to be a factor
```
```{r}
oxy.by.day$Genotype <- as.factor(oxy.by.day$Genotype) #Numbers don't mean anything
```

And now we need to drop the NAs, because that makes the Bayesian model upset
```{r}
library(tidyr) #the package we need
clean.oxy.by.day <- oxy.by.day %>% drop_na(oxy)
...
bayes_rm <- generalTestBF(oxy ~ Day.of.Exp * Temperature * Turbidity * Genotype + ID,
                          data = clean.oxy.by.day, whichRandom = "ID", neverExclude="^ID$")
```
...
plot(bayes_rm)
```
Alright. So our most likely model includes Day.of.Exp + Temperature + Day.of.Exp:Temperature + Day.of.Exp:Turbidity + Temperature:Turbidity + Genotype + ID
...
look.at.this <- bfmat[,167] #Prints out the 167th column, which should show everything divided by ID

data.this <- as.data.frame(look.at.this) #Makes it into a data frame, so hopefully we can interpret #that.

#write.csv(data.this, "TempTurbDayBayesOutput.csv")
```
So after all that it's 1596168206059.3 times more likely than a model that only includes ID

So let's see where genotypes come in
```{r}
library(readr)
library(ggplot2)
library(dplyr)
...
sum.oxy.by.day.genotype <- summarySE(oxy.by.day, measurevar = "oxy",
                       groupvars=c("Genotype", "Day.of.Exp"))

p3 <- ggplot(oxy.by.day, aes(x=Day.of.Exp, y = oxy))+
  
  geom_flat_violin(data=oxy.by.day, aes(x=factor(Day.of.Exp, levels = 1:3), y = oxy, 
                                        fill = factor(Genotype)), 
                   position = position_nudge(x = .25, y = 0), adjust = 1.5, trim = TRUE, alpha = .5, colour = NA)+
  geom_point(aes(x=factor(Day.of.Exp, levels = 1:3), y = oxy, 
                                    colour = factor(Genotype), shape = factor(Treatment)), 
             position = position_jitter(width = .2), size = 2)+
  geom_line(data = sum.oxy.by.day.genotype, aes(x = as.numeric(Day.of.Exp)+.25, y = oxy_mean, 
                                       group = factor(Genotype), colour = factor(Genotype)), 
            linetype = 3)+
  #scale_color_manual(values = c("black", "#1E90FF", "#000080", "#881B1B"))+
  #scale_fill_manual(values = c("black", "#1E90FF", "#000080", "#881B1B"))+
  scale_shape_manual(values = c(15, 17, 16, 18))+
  labs(color = "Genotype", fill= "Genotype", shape = "Treatment")+ 
  geom_hline(yintercept = 0, color = "black", linetype = "dashed")+
  #theme_cowplot()+
  theme(panel.background = element_rect(fill = "white", colour = "black"))+
  xlab("Day of Experiment")+
  ylab("Oxygen Produced mg/L/12 hrs")+
  ggtitle("Daytime Oxygen Production by Day and Genotype")
...
```{r}
ggsave(p3, 
       filename = "GenotypeTurbTempEasterRespirometryDaymare.png",
       device = "png")
```
Well that's ugly as an unfun sin, but maybe we need to see the night data before we completely write off genotype as something we consider.
