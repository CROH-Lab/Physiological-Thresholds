...
```
Looks like it gets it together as time goes on
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 7 Chamber 2") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
It also seems like respiration increases over time
...
write.csv(corrected.data, "Trial7Chamber2NoBG.csv")
