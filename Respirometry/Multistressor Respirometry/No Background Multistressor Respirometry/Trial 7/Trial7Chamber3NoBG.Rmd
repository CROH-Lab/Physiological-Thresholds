...
```
Seems like respiration increased as time wore on
...
```
Looks nice everywhere I can see
...
```
Looks like respiration increases over time
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 7 Chamber 3") + geom_hline(aes(yintercept=0)) +
...
#write.csv(corrected.data, "Trial7Chamber3NoBG.csv")
