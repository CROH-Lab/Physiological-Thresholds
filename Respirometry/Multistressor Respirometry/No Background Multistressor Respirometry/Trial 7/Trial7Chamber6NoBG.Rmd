...
```
More respiration on the first and third day I guess
...
```
I think that looks about how I expected
...
```
Photosynthesis gets worse over time
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 7 Chamber 6") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
I don't know what happened on day 3
...
#write.csv(corrected.data, "Trial7Chamber6NoBG.csv")
