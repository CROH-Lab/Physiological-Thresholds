...
```
Also seems to start out higher than it ends up
...
```
Although more photosynthesis on the first day it looks like
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 7 Chamber 5") + geom_hline(aes(yintercept=0)) +
...
#write.csv(corrected.data, "Trial7Chamber5NoBG.csv")
