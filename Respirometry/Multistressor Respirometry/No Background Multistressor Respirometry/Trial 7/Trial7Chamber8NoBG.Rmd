...
```
Everybody just respires more at the end
...
```
Or was respiration on night two just less than it should be
...
```
Another case of deteriorating oxygen production
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 7 Chamber 8") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
More time in the chamber doesn't result in a happier coral
...
#write.csv(corrected.data, "Trial7Chamber8NoBG.csv")
