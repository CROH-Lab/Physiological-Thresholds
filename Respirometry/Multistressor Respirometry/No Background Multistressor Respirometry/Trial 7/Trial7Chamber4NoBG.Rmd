...
```
Seems like less of an increase in respiration over time
...
```
Seems to become more cyclic over time

Although when they roll the rates things look more regular
...
```
Small amount of photosynthesis and increasing respiration
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 7 Chamber 4") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
Very rarely gets it right
...
#write.csv(corrected.data, "Trial7Chamber4NoBG.csv")
