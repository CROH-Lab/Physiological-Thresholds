...
```
So I think some of the early loops are iffy
...
```
Although the calculated rates look suspeciously normal in this situation
...
```
So maybe part of the problem is that this one didn't really do much
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 6 Chamber 7") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
Almost all net respiration
...
#write.csv(corrected.data, "Trial6Chamber7NoBG.csv")
