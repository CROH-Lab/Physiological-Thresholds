...
```
Finally something that looks like photosynthesis
...
```
This one actually looked happier on days 2 and 3

Definitely some indications of decreasing photosynthesis
...
```
17 continues to be the bane of everybody's existence
...
```
Alright. Actually more net photosynthesis earlier
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 6 Chamber 6") + geom_hline(aes(yintercept=0)) +
...
#write.csv(corrected.data, "Trial6Chamber6NoBG.csv")
