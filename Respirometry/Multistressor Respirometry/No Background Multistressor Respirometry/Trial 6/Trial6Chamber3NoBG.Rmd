...
```
Once again it looks like corals did not enjoy being in chambers
...
```
A few inappropriately timed flushes
...
```
Very little net photosynthesis
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 6 Chamber 3") + geom_hline(aes(yintercept=0)) +
...
#write.csv(corrected.data, "Trial6Chamber3NoBG.csv")
