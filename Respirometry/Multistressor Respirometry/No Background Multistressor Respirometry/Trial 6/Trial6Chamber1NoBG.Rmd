...
```
Seems like we're seeing a decline
...
Seems to look like I would expect
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 6 Chamber 1") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
Basically entirely net respiration
...
#write.csv(corrected.data, "Trial6Chamber1NoBG.csv")
