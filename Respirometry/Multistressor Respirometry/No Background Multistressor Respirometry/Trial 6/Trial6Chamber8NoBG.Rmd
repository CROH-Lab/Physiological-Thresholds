...
library(respR)
library(tidyr)
raw.data <- read.csv("Trial6Resp.csv") #load raw trial data
...
Trial <- 6 #Experiment labeled to prevent subsequent confustion
...
```
Things got a bit weird towards the end
...
```
Still a bit weird towards the end, but the rolling may fix it
...
```
Yeah, the end may get a bit weird
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 6 Chamber 8") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
Although most of the weirdness may be after 72

Now let's clean up so we can combine. That's where Briana's previous used and not used loops come in. Briana cut it at hour 51. Maybe I'll cut it at 48 and leave day 3 out of it.
```{r}
#Bringing in the old data
old.results <- read.csv("Trial6Results.csv")
...
#write.csv(corrected.data, "Trial6Chamber8NoBG.csv")
