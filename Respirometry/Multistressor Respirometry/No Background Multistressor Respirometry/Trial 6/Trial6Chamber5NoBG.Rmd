...
```
So it seems like the issue with 17 went across both units
...
```
Some net photosynthesis on the first day so that's nice
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 6 Chamber 5") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
I really don't think corals like being in the chambers
...
#write.csv(corrected.data, "Trial6Chamber5NoBG.csv")
