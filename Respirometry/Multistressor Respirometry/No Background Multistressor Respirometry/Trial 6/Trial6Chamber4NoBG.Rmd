...
```
Declining, but less dramatically so
...
```
Looks to be doing better than some available options
...
```
Also a badly timed flush at 17
...
```
Once again very little net photosynthesis
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 6 Chamber 4") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
One moment of net photosynthesis
...
#write.csv(corrected.data, "Trial6Chamber4NoBG.csv")
