...
```{r}
datos <- read.csv("Trial10NoBG.csv")
```
...
unique(datos$Time.of.Day) #We've got extras, which I don't need today
library(stringr) #The package we need 
datos$Time.of.Day <- str_trim(datos$Time.of.Day, side = c("both"))
unique(datos$Time.of.Day) #Better
...
unique(dia$Hour) #So that also looks okay, so what the hell went wrong with our
#combining in the other data sheet. I'll have to figure it out.
```
...
```
All negative, so actually fine, but I guess we can go through the motions to get a repirometry setup.
...
So I guess these ones can just go all back together for the combining.
