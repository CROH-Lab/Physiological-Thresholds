...
```
Gets a little weird towards the end
...
```
Got pretty weird, but it be that way sometimes
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 10 Chamber 5") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
So this one got real weird
...
#write.csv(corrected.data, "Trial10Chamber5NoBG.csv")
