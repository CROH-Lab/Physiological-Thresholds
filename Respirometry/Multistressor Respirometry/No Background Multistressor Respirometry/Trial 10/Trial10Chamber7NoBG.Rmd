...
library(respR)
library(tidyr)
raw.data <- read.csv("Trial10Resp.csv") #load raw trial data
...
Trial <- 10 #Experiment labeled to prevent subsequent confustion
...
#AutoResp names the columns "CH1.02...air.sat.." in the raw data set, you have to process the data one chamber at a time, this isolates the raw data for the Chamber
...
```
Looks like it increases over time, but that's alright.
...
m.end <- as.numeric(rownames(as.data.frame(coral.resp)[which(as.data.frame(coral.resp)$Phase=="M80  ", arr.ind=TRUE)[1],]))
...
```
Appears to go off the rails on day three
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 10 Chamber 7") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
This was a coral that seemed just generally unhappy in chambers
...
old.results <- read.csv("Trial10Results.csv")
...
corrected.data$Trial <- rep(Trial)

#Write a csv of our data. We'll put them all together later
#write.csv(corrected.data, "Trial10Chamber7NoBG.csv")
