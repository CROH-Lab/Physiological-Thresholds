...
```
It looks like this one may be troublesome.
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 10 Chamber 6") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
Although I do wonder if the relatively high starting values played a role.
...
#write.csv(corrected.data, "Trial10Chamber6NoBG.csv")
