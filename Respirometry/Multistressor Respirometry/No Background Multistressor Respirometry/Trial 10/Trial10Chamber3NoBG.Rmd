...
```
So maybe a little bit is happening
...
```
Let's see what we got once we flip things over
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 10 Chamber 3") + geom_hline(aes(yintercept=0)) +
...
#write.csv(corrected.data, "Trial10Chamber3NoBG.csv")
