...
```
So it looks like our oxygen did something interesting here
...
```
Tentatively looks like fewer pump issues, so I'll take it
...
Appears more believeable than trial 8
Making our graph
```{r}
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 10 Chamber 1") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
Cycles and tending towards net oxygen consumption
...
#write.csv(corrected.data, "Trial10Chamber1NoBG.csv")
