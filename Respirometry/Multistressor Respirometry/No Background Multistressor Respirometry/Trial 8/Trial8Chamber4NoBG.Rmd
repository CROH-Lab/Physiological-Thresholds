...
```
Looks somewhat erratic through the experiment and with fewer clear trends.
...
```
Still some poorly timed flushes as usual
...
```
Still looks like we got off to a weird start
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 8 Chamber 4") + geom_hline(aes(yintercept=0)) +
...
#write.csv(corrected.data, "Trial8Chamber4NoBG.csv")
