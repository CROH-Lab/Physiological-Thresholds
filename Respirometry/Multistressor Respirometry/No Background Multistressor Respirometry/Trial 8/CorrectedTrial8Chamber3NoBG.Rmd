---
title: "CorrectedTrial8Chamber3NoBG"
author: "Abby Cannon"
date: "2024-10-23"
...
```
So it seems like getting weirder as things go on is a trait here
...
```
There's a bit of apparent nighttime oxygen production from this angle, which I don't love
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 8 Chamber 3") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
More respiration as the experiment goes on
...
#write.csv(corrected.data, "CorrectedTrial8Chamber3NoBG.csv")
