---
title: "CorrectedTrial8Chmaber5NoBG"
author: "Abby Cannon"
date: "2024-10-24"
...
```
Looks like we had a bit of a chamber freakout, but less dramatic than some.
...
```
We may have had a few things going bump in the night
...
```
I think all of these are acceptable

Now we need a way keep track of our problems
```{r}
horas.malas <- c(3, 6, 8, 9, 10, 26, 53, 56, 59)
```
...
```
Some evidence of going off the rails, but less.
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 8 Chamber 5") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
Alright. That seems significantly less suspicious.
...
#write.csv(corrected.data, "CorrectedTrial8Chamber5NoBG.csv")
