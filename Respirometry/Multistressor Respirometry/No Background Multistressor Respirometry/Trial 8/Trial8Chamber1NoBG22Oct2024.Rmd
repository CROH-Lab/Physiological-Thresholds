...
```
Still tendency for respiration to increase over time
...
```
And for this round we'll have to trim coral.resp to prevent bad loops
```{r}
class(coral.resp$X) #Integer. That will work
#And data point 40147 is where the last flush happens so I think we'll have to hit 
#it there
coral.resp <- coral.resp[ which(coral.resp$X >= 40147), ]
...
```
Looks like this one was a bit weird at the start

```{r}

#so there may be a way to find this programmatically without poking through the data
#every time
#Although this time we have to start at F11 due to the system re-starting, which is
#frustrating
m.start <- as.numeric(rownames(as.data.frame(coral.resp)[which(as.data.frame(coral.resp)$Phase =="F11   ", arr.ind=TRUE)[1],]))-120
...
So if we operate on the assumption that coral.resp is missing some rows we should see how long it should be from the first measurement to the last if all the data is there
```{r}
box.census <- as.data.frame(table(coral.resp$Phase))
#So we have way too many M10s and too few M84s. The latter may not be a problem. 
```

So now let's get a closer look at what's going on there
```{r}
#Let's find out what we're working with in measurement.only
unique(measurement.only$Phase)
#So number followed by two spaces
diez <- measurement.only[ which(measurement.only$Phase=='10  '), ]
#Let's see if we can do some subtraction to see how long things actually ran for
#But first we need to make the data not character
#And to make it time we may need a specific package
library(chron) #The package
m10times <- chron(times = diez$Time) #Take them from characters to times
diez$Time <- m10times #replaces the old column
diez$Time[5934]-diez$Time[1] #That's a lot 01:44:38
...
And there are a lot of inappropriately timed early flushes, which explains a lot
...
So this one stayed positive, which is odd
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 8 Chamber 1") + geom_hline(aes(yintercept=0)) +
...
#write.csv(corrected.data, "Trial8Chamber1NoBG.csv")
#After we adjust the rates by more correct value
```
I think this one is going to be a weird one.
