---
title: "CorrectedTrial8Chamber2NoBG"
author: "Abby Cannon"
date: "2024-10-22"
...
```
Will take the same amount of trimming, but I'll get there
...
```
So the untrimmed version looks weird
...
```
Alright. Now we look better there too
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 8 Chamber 2") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
Odd, but maybe workable
...
#write.csv(corrected.data, "CorrectedTrial8Chamber2NoBG.csv")
