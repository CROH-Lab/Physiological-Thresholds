---
title: "CorrectedTrial8Chamber7NoBG"
author: "Abby Cannon"
date: "2024-10-25"
...
```
Less ridiculous after smoothing
...
```{r}
horas.malas <- c(1, 3, 8, 27, 29, 32, 35, 53, 56, 59, 62, 65)
```
...
```
I'm guessing the high points are the ones that were problems
...
ggplot(data_c1, aes(x=loop, y=mo2)) + geom_point() + theme_classic() +
  ggtitle("OFAV Trial 8 Chamber 7") + geom_hline(aes(yintercept=0)) +
  labs(x="Loops", y=expression(MO[2]~(mgO[2]~kg^-1~h^-1))) 
```
Bit of a mess, but I guess that's okay
...
#write.csv(corrected.data, "CorrectedTrial8Chamber7NoBG.csv")
