...
```{r}
fluoro <- aggregate(alpha ~ Coral.ID + Treatment + Genotype, 
...
bayes_rm <- generalTestBF(alpha ~ Treatment + Genotype, 
...
```{r}
#So now we make our Bayesian Models
bayes_rm.high <- generalTestBF(alpha ~ At.or.Above.Legal.Threshold + Genotype, 
                          data = fluoro, whichRandom = "Genotype", 
                          neverExclude = "Genotype")
```
