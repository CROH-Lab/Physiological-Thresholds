...
```{r}
fluoro <- aggregate(ETRm ~ Coral.ID + Treatment + Genotype, 
...
bayes_rm <- generalTestBF(ETRm ~ Treatment + Genotype, 
...
```{r}
#So now we make our Bayesian Models
bayes_rm.high <- generalTestBF(ETRm ~ At.or.Above.Legal.Threshold + Genotype, 
                          data = fluoro, whichRandom = "Genotype", 
                          neverExclude = "Genotype")
```
...
look.at.this
```
1.32 times as likely. This feels like the Bayesian equivalent of p-hacking, but here we are.
...
#And then we make our graph
p2 <- ggplot(fluoro, aes(x = At.or.Above.Legal.Threshold, y = ETRm, label = Genotype))+
  geom_flat_violin(data= fluoro, aes(x= At.or.Above.Legal.Threshold, 
                                          y = ETRm, 
                                        fill = etiqueta), 
                   position = position_nudge(x = 0, y = 0), adjust = 1.5, trim = TRUE, alpha = .5, colour = NA)+
  geom_text_repel(aes(x= At.or.Above.Legal.Threshold, y = ETRm, 
                                    colour = etiqueta), 
  ...
  ggtitle("ETRm By Turbidity Levels")
...
```{r}
ggsave(p2, 
       filename = "ETRmPhase1AfterBetterLabelsBetterAxesTreatment.png",
       device = "png")
```
