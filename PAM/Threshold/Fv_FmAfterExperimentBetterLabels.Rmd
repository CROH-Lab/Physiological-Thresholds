...
p2 <- ggplot(fluoro, aes(x = Treatment, y = Fv.Fm, label = Genotype))+
  ...
  ylab("Fv.Fm")+
  ggtitle("Fv/Fm By Turbidity Treatment")
...
```{r}
#So now we make our Bayesian Models
bayes_rm.factor <- generalTestBF(Fv.Fm ~ At.or.Below.Legal.Threshold + Genotype, 
                          data = fluoro, whichRandom = "Genotype", 
                          neverExclude = "Genotype")
```
...
```{r}
ggsave(p2, 
       filename = "Fv_FMPhase1AfterBetterLabels.png",
       device = "png")
```
